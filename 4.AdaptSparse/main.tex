...
\entmaxtext function this work builds upon (\secref{sec:entmax_bg}). In their work, there is a
...
\eqnref{eq:final_gradient_alpha} provides the needed piece that was missing
...
\begin{figure}[ht]
    ...
    \caption[Trajectories of $\alpha$ values for a subset of the heads during
        ...
        training on \langp{en}{de}. Initialized at random, most heads become denser in the
...
seen in \figref{fig:hist_alphas}. We observe that the encoder
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption[Distribution of learned $\alpha$ values per attention block.]{
...
\begin{figure}[htbp]
    ...
    \caption[BPE-merging head $(\alpha=1.91)$ discovered in the
        $\alpha$-\entmaxtext model.]{%layer 5+1 / head 6+1, alpha=1.33.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption[Interrogation-detecting heads in the three models.]{%layer 0+1 head 2+1 alpha=1.05
...
\begin{figure}[htbp]
    ...
    \caption[Example of two sentences of similar length with different sparsity.]{%layer 5+1 / head 6+1, alpha=1.33.
...
whole probability weight to a single token in the sequence.
...
confidence of $93.5\%$. The $1.5$-\entmaxtext model also has three heads
for this position, with median confidence of $94.4\%$. The adaptive
model has four heads, with median confidences of $95.9\%$, the
lowest-confidence head being dense with $\alpha=1.18$, while the
highest-confidence head is sparse ($\alpha=1.91$).
...
of transformers, inspired by our analysis (\secref{sec:spec}) along with other
