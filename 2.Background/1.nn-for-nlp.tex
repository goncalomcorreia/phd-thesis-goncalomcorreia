...
\end{equation}
%
% some abuse of notation use the same letter p, OR subscript the pmf
% p_{Y|X} and p_{X}
% in some appl these are independent components that share some parameters
% in others they are marginals of the same joint distribution
%
% how transfer can be learning log p(x) and then use it to learn p(y|x)
% with same \theta
where
$\mathbbm{1}\left[(\bm{x}_n,y_n)\in\mathcal{D}_{\text{labeled}}\right]$
