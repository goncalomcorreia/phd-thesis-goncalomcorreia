...
%
where $x$ is an
observation (\eg, an MNIST image), $h$ is a continuous latent variable
...
notation in \secref{sec:lvm}, we let $\theta_\ell = \{\lambda,
    \phi\}$ and define $\pi(z|x; \theta_\pi) \defeq q(z|x; \lambda)$ and
the loss in \eqnref{eq:ss_loss_vae}
turns into
%
\begin{equation}
    L_{\mathcal D}(\theta) =
...
\end{align}
%
Finally, to complete the specification of \eqnref{eq:ss_loss_vae_vi},
