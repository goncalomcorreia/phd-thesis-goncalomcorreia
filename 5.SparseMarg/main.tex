...
\noindent Training with discrete variables (\secref{sec:discrete_lvm_bg}) can
...
60K datapoints for training and 10K datapoints for testing. We
perform model selection on the last 10K datapoints of the training
...
the correct image\,---\,only the correct image. To make the game even % TODO: do triple - like here
...
% TODO: fix the middle column here
\begin{table}[t]
    \begin{center}
        \tlfstyle
        \begin{tabular}{lr@{~}lr}
            \toprule
            Method                                 & \multicolumn{2}{r}{Comm. success (\%)} & Decoder calls                   \\
            \midrule
            {\emph{Monte Carlo}}                   &                                        &                           &     \\
...
independent categorical distribution for each pixel, $p(x |z,
    \phi) = \prod_{i=1}^{28} \prod_{j=1}^{28} p(x_{ij} |z, \phi)$.
...
\noindent We described a novel training strategy for discrete latent variable
