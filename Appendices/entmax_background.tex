...
\fancychapter{Background: \texorpdfstring{\boldmath $\alpha$-\entmaxtext}{alpha-entmax}}\label{app:fenchelyoung}
...
\section*{Characterizing the {\boldmath $\alpha$-\entmaxtext} mapping}\label{sec:bgform}
...
\section*{Connections to softmax and sparsemax}\label{sec:softmax}
...
$\alpha$-\entmaxtext expression (\eqnref{eq:entmax_form_supp}); for other
...
slackness. Thus, the solution must have the form $\pp_i =
    \nicefrac{\exp(\xx_i)}{Z}$, yielding \eqnref{eq:softmax-supp}.
